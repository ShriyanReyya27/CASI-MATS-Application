...
My Attack Framework (based on the \textit{automated} decomposition in \cite{jones2024adversariesb}):
\begin{enumerate}
    \item Generate a list of decomposed queries that alternate between frontier and weak model.

    I did this using the GPT-4.1 in the OpenAI API. Instead of writing code to generate the outputs, I chose to prompt within the built-in dashboard because it was easier to adapt queries if necessary. I specified in the prompt that reprompting is possible if certain things fail in the first round. This more closely follows the attack in \cite{jones2024adversariesb} because of the adaptability, but doing it manually means it is not scalable. However, for the purpose of a couple of attacks, I found it sufficient.
